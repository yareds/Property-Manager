...
export const PaymentPanel: React.FC<PaymentPanelProps> = ({
  ...
  // Helper to open modal for new payment
  ...
  // Helper to open modal for editing an existing payment
  const handleOpenEditModal = (p: Payment) => {
    setEditingPayment(p);
    setSelectedLeaseId(p.leaseId || '');
    setDueDate(p.dueDate || '');
    setPrepaymentPeriod(p.prepaymentPeriod || '1 Month (Standard)');
    setRentPeriodStart(p.rentPeriodStart || '');
    setRentPeriodEnd(p.rentPeriodEnd || '');
    setAmountDue(p.amountDue);
    setAmountPaid(p.amountPaid);
    setPaymentStatus(p.paymentStatus);
    setPaymentMethod(p.paymentMethod || 'ACH/Direct Deposit');
  ...
  // Lease Change Warning Confirmation State
  const [leaseChangeConfirm, setLeaseChangeConfirm] = useState<{
    isOpen: boolean;
    newLeaseId: string;
  } | null>(null);
  ...
  // Handle Record/Add or Update payment
  const handleRecordPayment = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedLeaseId || !dueDate || isSubmittingPayment) return;
    ...
    try {
      ...
      const receiptNum = editingPayment?.receiptNumber || `RCP-${crypto.randomUUID().slice(0, 8).toUpperCase()}`;

      if (editingPayment) {
        // Update existing payment
        const updatedPayment: Payment = {
          ...editingPayment,
          tenantId: lease ? lease.tenantId : editingPayment.tenantId,
          businessName: lease ? lease.businessName : editingPayment.businessName,
          leaseId: lease ? lease.id : editingPayment.leaseId,
          propertyId: lease ? lease.propertyId : editingPayment.propertyId,
          unitId: lease ? lease.unitId : editingPayment.unitId,
          unitNumber: lease ? lease.unitNumber : editingPayment.unitNumber,
          propertyName: lease ? lease.propertyName : editingPayment.propertyName,
          dueDate,
          paymentDate: amountPaid > 0 ? (editingPayment.paymentDate || new Date().toISOString().split('T')[0]) : undefined,
  ...
  const handleQuickMarkPaid = async (p: Payment) => {
    const receiptNum = `RCP-${crypto.randomUUID().slice(0, 8).toUpperCase()}`;
    ...
    <div className="space-y-4">
          ...
          >
            <div className="px-4 py-2.5 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
              <h3 className="font-bold text-xs uppercase tracking-wider text-slate-800 flex items-center space-x-1.5">
                ...
                <span>{editingPayment ? 'Edit Payment Record' : 'Record Rent Receipt'}</span>
              </h3>
              <button
            ...
            <form onSubmit={handleRecordPayment} className="p-4 space-y-3">
              ...
              <div className="space-y-0.5">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (hasManualEdits && newLeaseId !== selectedLeaseId) {
                      setLeaseChangeConfirm({
                        isOpen: true,
                        newLeaseId
                      });
                    } else {
                      setSelectedLeaseId(newLeaseId);
                      setHasManualEdits(false);
                      setPrepaymentPeriod('1 Month (Standard)');
                      const lease = leases.find(l => l.id === newLeaseId);
                      if (lease) {
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-0.5">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      setHasManualEdits(true);
                      setRentPeriodEnd(e.target.value);
                    }}
                    className="w-full px-2.5 py-1.5 text-xs border border-slate-200 rounded focus:outline-none focus:border-blue-500"
                  />
                </div>
              ...
              <div className="pt-2 flex items-center justify-end space-x-2">
                ...
                >
                  {isSubmittingPayment ? (editingPayment ? 'Saving...' : 'Recording...') : (editingPayment ? 'Update Payment' : 'Record Receipt')}
                </button>
              </div>
            </form>
          </motion.div>
      ...
      {deleteConfirm && (
        <ConfirmModal
          ...
          message={`Are you sure you want to delete this payment record of Br ${deleteConfirm.amount.toLocaleString()} for tenant "${deleteConfirm.tenantName}"? This action cannot be undone.`}
          confirmLabel="Delete Payment"
          cancelLabel="Cancel"
          isDanger={true}
          onConfirm={() => {
            deletePayment(deleteConfirm.paymentId);
      ...
      {/* Lease Change Override Warning Modal */}
      {leaseChangeConfirm && (
        <ConfirmModal
          isOpen={leaseChangeConfirm.isOpen}
          title="Replace Manual Edits?"
          message="Changing the selected tenant lease will replace the custom amount and date values you entered with default values from the new lease. Do you want to proceed?"
          confirmLabel="Replace Values"
          cancelLabel="Keep Current Edits"
          isDanger={false}
          onConfirm={() => {
            const newLeaseId = leaseChangeConfirm.newLeaseId;
            setSelectedLeaseId(newLeaseId);
            setHasManualEdits(false);
            setPrepaymentPeriod('1 Month (Standard)');
            const lease = leases.find(l => l.id === newLeaseId);
            ...
            setLeaseChangeConfirm(null);
          }}
          onClose={() => setLeaseChangeConfirm(null)}
        />
      )}
    </div>
  );
