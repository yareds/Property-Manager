...
export const DashboardOverview: React.FC<DashboardOverviewProps> = ({
  ...
  const unitStatuses = units.map(u => getUnitOccupancyAndTenant(u, leases, tenants).occupancyStatus);
  const occupiedUnits = unitStatuses.filter(s => s === 'Occupied').length;
  const vacantUnits = unitStatuses.filter(s => s === 'Vacant').length;
  const maintenanceUnits = unitStatuses.filter(s => s === 'Maintenance').length;
  const reservedUnits = unitStatuses.filter(s => s === 'Reserved').length;
  ...
  const upcomingExpirations = leases.filter(l => {
    ...
    const parts = l.endDate.split('-').map(Number);
